...
def backtest_DDQN_agent(config, logger, model_path):
    ...
    for step in range(max_steps):
        ...
        next_state, reward, done, info = trading_environment.unwrapped.trading_env_step(action)
        #next_state, reward, done, _ = trading_environment.unwrapped.trading_env_step(action)
        nav = info.get('nav')
        equity_curve.append(nav)
        logger.info(f"[Step {step:03d}] action={action}, reward={reward:.4f}, nav={nav:.4f}")
