...
class NewsDataDownloader:
    def __init__(self, config, logger):
        ...
        self.news_limit_per_api_call = config['ingestion'].get('news_limit_per_api_call', 100)
        ...
        self.api_calls_per_minute_limit = config['ingestion'].get('api_calls_per_minute_limit', 5)
    ...
    def create_news_stats(self):
        ...
        try:
            ...
            for symbol in self.symbols:
                ...
                if earliest_date and isinstance(earliest_date, str) and 'T' in earliest_date:
    ...
    def _make_api_call(self, url, params):
        ...
        while self._is_rate_limited() and self.recent_api_calls:
            self.logger.warning(f"API rate limit of {self.api_calls_per_minute_limit} calls per minute reached.")
        ...
        if self.api_call_count >= self.api_call_total_limit:
            self.logger.warning(f"API call total limit reached ({self.api_call_total_limit}). Cannot make more calls.")
            return None
    ...
    def download_ticker_news(self, symbol):
        ...
        """
        ticker_news = []
        batch_start_datetime = datetime.strptime(self.start_datetime, '%Y-%m-%d %H:%M:%S')
        config_end_datetime = datetime.strptime(self.end_datetime, '%Y-%m-%d %H:%M:%S')
        
        self.logger.info(f"Downloading news for {symbol} from {batch_start_datetime} to {config_end_datetime}")
        ...
        while (config_end_datetime - batch_start_datetime).total_seconds() >= 60:
            ...
            if 'feed' not in data or len(data['feed']) == 0:
                self.logger.warning(f"No news found for {symbol} in the date range {batch_start_datetime} to {config_end_datetime}")
                break
            ...
            for item in data['feed']:
                news = {
                    'symbol': symbol,
                    'datetime': datetime.strptime(item['time_published'], '%Y%m%dT%H%M%S'),
                    'title': item['title'],
                    'source': item['source'],
            ...
            if batch_size < int(self.news_limit_per_api_call):
                self.logger.info(f"Retrieved {batch_size} news items (fewer than limit). No more pagination needed.")
                break
    ...
    def download_news(self):
        ...
        for symbol in self.symbols:
            # Check if we've reached the API call limit
            if self.api_call_count >= self.api_call_total_limit:
                self.logger.warning(f"API call total limit reached ({self.api_call_total_limit}). Stopping news download.")
                break
        ...
        # Log the total count
        if n_news_data > 0:
            self.logger.info(f"Downloaded and stored a total of {n_news_data} news items with {self.api_call_count} API calls")
            return
        else:
            self.logger.info(f"No news data downloaded with {self.api_call_count} API calls")
            return 
